...
namespace brt
{
#pragma region hsv and hsl colors
	...
	{
		// algorithm: https://stackoverflow.com/questions/3018313/algorithm-to-convert-rgb-to-hsv-and-hsv-to-rgb-in-range-0-255-for-both
		float hh, p, q, t, ff;
		long i;
		color out;
	...
	float HueToRGB(float v1, float v2, float vH)
	{
		...
		if ((6 * vH) < 1.f) { return (v1 + (v2 - v1) * 6.f * vH); }
		if ((2.f * vH) < 1.f) { return v2; }
		if ((3.f * vH) < 2.f) { return (v1 + (v2 - v1) * ((2.0f / 3.f) - vH) * 6.f); }
	...
	{
		// algorithm: https://www.programmingalgorithms.com/algorithm/hsl-to-rgb?lang=C%2B%2B
...
#pragma region color
	...
	/// doesn't divide alpha values with each other, just returns a color with alpha 255
	color & color::operator/=(float rhs)
	{
		*this = *this / rhs;
		return *this;
