...
namespace brt
{
	...
	void scenewhitted::build_scene()
	{
		...
		// glass sphere
		std::shared_ptr<material> glassspheremat = std::static_pointer_cast<material>(std::make_shared<materialdefault>(sf::Color(230, 230, 230, 80), 200.f, sf::Color(150, 150, 150), 0.f, true, /*0.97f*/ REFRACTIVEGLASS));
		m_objects.push_back(new sphere(vec3(0.3f, 2.12f, -5.f), 0.95f, glassspheremat)); // original pos (0.3f, 2.12f, -5.f)

		// inner glass sphere
		std::shared_ptr<material> innerglassspheremat = std::static_pointer_cast<material>(std::make_shared<materialdefault>(sf::Color(230, 230, 230, 80), SPECNONE, sf::Color::White, 0.f, true, 0.65f));
		m_objects.push_back(new sphere(vec3(0.3f, 2.12f, -5.f), 0.9495f, innerglassspheremat)); // original pos (0.3f, 2.12f, -5.f)

		// metal sphere
		std::shared_ptr<material> metalspheremat = std::static_pointer_cast<material>(std::make_shared<materialdefault>(sf::Color(190, 201, 160), SPECMIDLYSHINY, sf::Color::White, 0.5f));
		m_objects.push_back(new sphere(vec3(-1.f, 1.1f, -7.5f), 0.9f, metalspheremat));

		// checkerboard quadrilateral
		std::shared_ptr<material> quadmat = std::static_pointer_cast<material>(std::make_shared<materialproccheckerboard>(1.5f, sf::Color(255, 0, 0), sf::Color(255, 255, 0)));
